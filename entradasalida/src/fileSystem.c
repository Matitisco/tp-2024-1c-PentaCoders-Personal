...
void levantar_archivo_bloques()
{
    ...
    if (archivo_bloque == -1)
    {
        archivo_bloque = open(path_archivo_bloques, O_CREAT | O_RDWR, S_IRUSR | S_IWUSR);
...
void mover_archivo_al_final_del_fs(char *nombre_archivo)
{
    ...
    bloques_archivo = obtener_bloques_archivo(nombre_archivo);                   // TODO
    cantidad_bloques_archivo = obtener_cantidad_bloques_archivo(nombre_archivo); // TODO
...
int obtener_cantidad_bloques_archivo()
{
    log_info(logger, "TODO FUNCTION OBTENER CANTIDAD BLOQUES ARCHIVOS");
    return -1;
}
...
{
    log_info(logger, "NO ESTA HECHO DESPLAZAR ARCHIVOS Y ELIMINAR BLOQUES LIBRES2");
    /*     int total_bloques = valores_config->block_count;
        ...
        while (bloque_actual < total_bloques)
        {
            ...
            if (bitarray_test_bit(bitarray, bloque_actual))
            {
                ...
                if (bloque_libre < bloque_actual)
                {
                    ...
                    actualizar_metadata_archivo(bloque_actual, bloque_libre);
